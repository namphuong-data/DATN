...
def process_silver():
    ...
    try:
        ...
        df_cleaned = df.filter(
            ...
            & (F.year("tpep_pickup_datetime") == F.year("tpep_dropoff_datetime"))
            & (F.month("tpep_pickup_datetime") == F.month("tpep_dropoff_datetime"))
        ...
        log.info("[SILVER] Đang tiến hành VACUUM để thu hồi dung lượng...")

        # Cho phép Vacuum với khoảng thời gian giữ lại bằng 0 (mặc định Delta chặn việc này để an toàn)
        ...
        # vacuum(0) sẽ xóa sạch tất cả các file cũ không thuộc phiên bản hiện tại
        silver_table.vacuum(0)
        log.info("[SILVER] ✓ Đã dọn dẹp xong các file dữ liệu cũ.")
        ...
        try:
            ...
            df_check.show(1, truncate=False, vertical=True)
        except Exception as e:
            log.warning("[SILVER] Không thể đọc lại Silver để kiểm tra: %s", e)
        finally:
            spark.stop()
